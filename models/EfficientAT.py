""" EfficientAT: https://github.com/fschmid56/EfficientAT/blob/main/models/MobileNetV3.py"""
...
class MobileNetV3(nn.Module, audobject.Object):
    def __init__(
        self,
    ...
    def _forward_impl(self, x: Tensor) -> (Tensor, Tensor):
        ...
        x = x.permute(0, 1, 3, 2) # swap tdim with fdim: [B, 1, F, T]
        x = self.features(x)
...
if __name__ == '__main__':
    ...
    #mdl = MobileNetV3(num_classes=1)
    mdl = get_model(width_mult=1.0, pretrained_name="mn10_as", num_classes=1,
                    input_dim_t=input_tdim, input_dim_f=n_mels)
    # input a batch of 10 spectrogram, each with 100 time frames and 128 frequency bins
    test_output = mdl(test_input)
    # output should be in shape [10, 1], i.e., 10 samples, each with prediction of 527 classes.
